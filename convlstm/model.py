...
class ConvLSTM_model(object):
    def __init__(self,
        ...
        self.convlstm_dim = convlstm_dim
        ...
        self.visual_feat = self.visual_feat_c5
    ...
    def train_op(self):
        ...
        if self.finetune_cnn:
            tvars += [var for var in tf.trainable_variables if var.op.name.startswith('res5')
                or var.op.name.startswith('res4') or var.op.name.startswith('res3')]
