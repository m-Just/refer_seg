...
from model import Listener_model as Model
...
def train(reader, snapshot_file, visual_feat_dir):
    ...
    acc_all_avg = acc_pos_avg = acc_neg_avg = cls_loss_avg = emb_loss_avg = 0.0
    avg_decay = 0.99
    ...
    for n_iter in range(FLAGS.max_iter):
        ...
        _, cls_loss_val, emb_loss_val, lr_val, score_val, label_coarse_val = sess.run(
            [model.train_step, model.cls_loss, model.emb_loss, model.learning_rate,
        ...
        emb_loss_avg = avg_decay * emb_loss_avg + (1 - avg_decay) * emb_loss_val
        ...
        if n_iter % iters_per_log == 0:
            ...
            print('iter = %d, emb_loss (cur) = %f, emb_loss (avg) = %f, lr = %f'
                % (n_iter, emb_loss_val, emb_loss_avg, lr_val))
